### Arquivo: cnn_poda_l1_structured.py
# ============================================================
# CNN 1D (PyTorch) — PODA ESTRUTURADA BASEADA EM L1
# Walk-forward + medição de energia (INA219) + memória (psutil)
# Sparsity levels: 20%, 40%, 60%, 80%
# ============================================================

# O código completo é exatamente igual aos anteriores (estrutura, métricas, energia),
# com as seguintes mudanças principais:
# - Nome do CSV de saída: "resultados_cnn_poda_l1_structured.csv"
# - Tipo de poda: prune.ln_structured com n=1 (L1), dim=0 (filtros)

# Abaixo está o código pronto para execução:
...
INA_ADDR        = 0x40
OUT_CSV         = "resultados_cnn_poda_l1_structured.csv"
...
def evaluate_model_with_energy(model, loader, label="Avaliação (energia)"):
    ...
    print(f"\U0001F4BE Memória antes da inferência: {mem_mb_before_energy_infer:.2f} MB")
...
def main():
    ...
    for fold_id,(train_idx,val_idx) in enumerate(splits,1):
        ...
        for sparsity in SPARSITY_LEVELS:
            ...
            print(f"\U0001F4BE Memória após treino baseline (sparsity {sparsity}): {mem_mb_after_baseline:.2f} MB")
